/*
 * 1. 8x8 SAD merge
 * 2. Two step and Square search Algorithm: 16x16 + (2+1)*(2+1)
 */
#include "x264-cuda.h"
...
//	dim3    threads(me_range*2, me_range*2);
	...
	me<<<blocks, Q_THREADS_PER_BLOCK>>>( c->i_pixel, c->dev_fenc_buf, c->dev_fref_buf, c->me, me_range, stride_buf);
 ...
 * *****************************************************************/
__device__ void me_merge_16x16( int sadCache[4][Q_THREADS_PER_BLOCK], int sadMerge[Q_THREADS_PER_BLOCK], int index[THREADS_PER_BLOCK], x264_cuda_me_t *me, int me_range, pixel *p_fenc, pixel *p_fref, int stride_buf)
{
	__shared__ int sadSquare[9];
...
//		__syncthreads();
		...
		if (offset < 9)
		{
			int omx = index[0] % me_range * 2 - 1 + offset%3;
			int omy = index[0] / me_range * 2 - 1 + offset/3;
		...
		if (offset == 0)
		{
			int minSAD = sadMerge[ index[0] ];
			int minIndex = 4;
			for(int i = 0; i < 9; i++)
			{
				if(sadSquare[i] < minSAD)
				{
					minIndex = i;
					minSAD = sadSquare[i];
				}
			}
			me[mb_x + mb_y*mb_width].mvc16x16.mv[0] = index[0] % me_range * 2 - me_range + minIndex%3 -1;
			me[mb_x + mb_y*mb_width].mvc16x16.mv[1] = index[0] / me_range * 2 - me_range + minIndex/3 - 1;
			me[mb_x + mb_y*mb_width].mvc16x16.cost = minSAD;
		}
		__syncthreads();
	}
}
 ...
 * 2. find least SAD and corresponding MV
 * *****************************************************************/
__device__ void me_merge_16x8( int sadCache[4][Q_THREADS_PER_BLOCK], int sadMerge[Q_THREADS_PER_BLOCK], int index[Q_THREADS_PER_BLOCK], x264_cuda_me_t *me, int me_range, pixel *p_fenc, pixel *p_fref, int stride_buf )
{
	__shared__ int sadSquare[9];
	const int offset = threadIdx.x;
	int mb_x = blockIdx.x;
	...
	for(int i16x8 = 0; i16x8 < 2; i16x8++)
	{
		...
		// for reductions, Q_THREADS_PER_BLOCK must be a power of 2
		// because of the following code： find least SAD
		int i = blockDim.x/2;
		while (i != 0) {
			if (offset < i)
...
//		__syncthreads();
		...
		if (offset == 0)
		{
			...
			me[mb_x + mb_y*mb_width].mvc16x8[i16x8].mv[0] = index[0] % me_range * 2 - me_range + minIndex%3 -1;
			me[mb_x + mb_y*mb_width].mvc16x8[i16x8].mv[1] = index[0] / me_range * 2 - me_range + minIndex/3 - 1;
			me[mb_x + mb_y*mb_width].mvc16x8[i16x8].cost = minSAD;
		}
		__syncthreads();
	}
}
 ...
 * 2. find least SAD and corresponding MV
 * *****************************************************************/
__device__ void me_merge_8x16( int sadCache[4][Q_THREADS_PER_BLOCK], int sadMerge[Q_THREADS_PER_BLOCK], int index[Q_THREADS_PER_BLOCK], x264_cuda_me_t *me, int me_range, pixel *p_fenc, pixel *p_fref, int stride_buf )
{
	__shared__ int sadSquare[9];
	const int offset = threadIdx.x;
	int mb_x = blockIdx.x;
...
//		__syncthreads();
		...
		if (offset < 9)
		{
			...
			pixel *p_ref = p_fref + omx+ omy * stride_buf + ( i8x16 * 8);
		...
		if (offset == 0)
		{
			...
			me[mb_x + mb_y*mb_width].mvc8x16[i8x16].mv[0] = index[0] % me_range * 2 - me_range + minIndex%3 -1;
			me[mb_x + mb_y*mb_width].mvc8x16[i8x16].mv[1] = index[0] / me_range * 2 - me_range + minIndex/3 - 1;
			me[mb_x + mb_y*mb_width].mvc8x16[i8x16].cost = minSAD;
		}
		__syncthreads();
	}
}
...
__global__ void me( int i_pixel, pixel *dev_fenc_buf, pixel *dev_fref_buf, x264_cuda_me_t *me, int me_range, int stride_buf) {
	 __shared__ int sadCache[4][Q_THREADS_PER_BLOCK]; // 4k
	 __shared__ int sadMerge[Q_THREADS_PER_BLOCK]; // 1k
	 __shared__ int sadSquare[9];
	 __shared__ int index[Q_THREADS_PER_BLOCK]; // 1k
...
//		__syncthreads();
		...
		if (offset < 9)
		{
			...
			ox = index[0] % me_range * 2 - 1 + offset%3;
			oy = index[0] / me_range * 2 - 1 + offset/3;
		...
		if (offset == 0)
		{
			int minSAD = sadCache[i8x8][ index[0] ];
			int minIndex = 4;
			for(int i = 0; i < 9; i++)
			{
				if(sadSquare[i] < minSAD)
...
//			me[mb_x + mb_y*mb_width].mvc8x8[i8x8].mv[1] = index[0] / ( me_range*2 ) - me_range;
//			me[mb_x + mb_y*mb_width].mvc8x8[i8x8].cost = sadCache[i8x8][ index[0] ];
			me[mb_x + mb_y*mb_width].mvc8x8[i8x8].mv[0] = index[0] % me_range * 2 - me_range + minIndex%3 -1;
			me[mb_x + mb_y*mb_width].mvc8x8[i8x8].mv[1] = index[0] / me_range * 2 - me_range + minIndex/3 - 1;
			me[mb_x + mb_y*mb_width].mvc8x8[i8x8].cost = minSAD;
		}
		__syncthreads();
	}
...
__global__ void me0( int i_pixel, pixel *dev_fenc_buf, pixel *dev_fref_buf, x264_cuda_mvc_t *p_mvc16x16, int me_range, int stride_buf) {
	 __shared__ int sadCache[Q_THREADS_PER_BLOCK]; // 4k
	 __shared__ int index[Q_THREADS_PER_BLOCK]; // 4k
...
__global__ void me_shared_cache( int i_pixel, pixel *dev_fenc_buf, pixel *dev_fref_buf, x264_cuda_mvc_t *p_mvc16x16, int me_range, int stride_buf) {
	...
	x = (offset + Q_THREADS_PER_BLOCK) % (16*3);
	y = (offset + Q_THREADS_PER_BLOCK) / (16*3);
	mb_ref[x + y*stride_ref] = p_fref[x + y*stride_buf];

	if(offset + 2*Q_THREADS_PER_BLOCK < 16*3 * 16*3)
	{
		x = (offset + 2*Q_THREADS_PER_BLOCK) % (16*3);
		y = (offset + 2*Q_THREADS_PER_BLOCK) / (16*3);
		mb_ref[x + y*stride_ref] = p_fref[x + y*stride_buf];
	}
...
__global__ void me_two_mb( int i_pixel, pixel *dev_fenc_buf, pixel *dev_fref_buf, x264_cuda_mvc_t *p_mvc16x16, int me_range, int stride_buf) {
	...
	mb_ref[x + y*stride_ref] = p_fref[x + y*stride_buf];
	x = (offset + Q_THREADS_PER_BLOCK) % stride_ref;
	y = (offset + Q_THREADS_PER_BLOCK) / stride_ref;
	mb_ref[x + y*stride_ref] = p_fref[x + y*stride_buf];

	x = (offset + 2*Q_THREADS_PER_BLOCK) % stride_ref;
	y = (offset + 2*Q_THREADS_PER_BLOCK) / stride_ref;
	mb_ref[x + y*stride_ref] = p_fref[x + y*stride_buf];
